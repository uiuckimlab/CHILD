...
controller = DEVICES_DICT[device_config.type](robot_config, device_config)
class Runner():
    def __init__(self, device_config):
        self.controller= controller
        self.controller.launch_init(np.zeros(14))
        self.shutdown = False
    ...
    def run(self):
        print("Press Enter to zero pose")
        ...
        print("Zero Pose Done, Press Enter to intialize pose")
        ...
        self.command = initial_command[0]
        # interpolate to initial command
        init_duration = 3.0
        target_qpos = np.concatenate([initial_command[0]['left_leg'],initial_command[0]['right_leg'],initial_command[0]['left_arm'],initial_command[0]['right_arm']])
        time_ = 0
        ...
        status_str = "Running... "
        bar_list = ["|", "|", "|", "|", "/", "/", "/", "/", "-", "-", "-", "-", "\\", "\\", "\\", "\\"]
        while not self.controller.require_end:
            ...
            if self.prev_target_qpos is not None:
                ...
                #trget_qpos[abs(diff) > 3.14] = self.prev_target_qpos
            start_time = time.time()
            self.low_cmd.motor_cmd[G1JointIndex.kNotUsedJoint].q = 0  # 1:Enable arm_sdk, 0:Disable arm_sdk
            self.low_cmd.mode_pr = Mode.PR
            self.low_cmd.mode_machine = self.mode_machine_
