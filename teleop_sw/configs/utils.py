...
def sanity_check_device_config(device_config, robot_config, verbose=True):
    if device_config.output_type not in ['joint_pos', 'delta_eef_pose']:
        raise ValueError(f"Invalid output type: {device_config.output_type}. Must be 'joint_pos' or 'delta_eef_pose'.")
    ...
    if 'child' in device_config.type:
        if len(device_config.direct_mapping) == 0:
            ...
            for leader_limb_name in device_config.limb_joint_names.keys():
                device_config.direct_mapping[leader_limb_name] = leader_limb_name

        direct_mapping_possible = robot_name in device_config.direct_mapping_available_robots
        if direct_mapping_possible:
            device_config.limb_mapping = {}
            for leader_limb_name in device_config.limb_joint_names.keys():
                follower_limb_name = device_config.direct_mapping[leader_limb_name]
                ...
                device_config.limb_mapping[leader_limb_name] = follower_limb_name
        elif not direct_mapping_possible:
            if device_config.output_type == 'joint_pos':
                print("Warning: Direct joint mapping is not available for this robot. Changing to eef mapping.")
                device_config.output_type = 'delta_eef_pose'
                print("- device_config.output_type: ", device_config.output_type)

            leader_limb_names = list(device_config.limb_joint_names.keys())
            ...
            for leader_limb_name, follower_limb_name in zip(leader_limb_names, follower_limb_names):
                device_config.limb_mapping[leader_limb_name] = follower_limb_name
            # answer = input(f"Trying to map limbs {leader_limb_names} to {device_config.limb_mapping.values()}. Do you want to change the mapping? [y/n]: ")
            # if answer.lower() == 'y':
            ...
            #                 continue
            #             else:
            #                 device_config.limb_mapping[leader_limb_name] = answer
            #                 print(f"Mapping {leader_limb_name} to {answer}")
            #                 break
            # device_config.limb_mapping['robot1'] = 'right_arm'
            # device_config.limb_mapping['robot2'] = 'left_arm'

            print("Mapping completed.")
            print("- device_config.limb_mapping: ", device_config.limb_mapping)



    # [TODO] add more sanity check for device_config
